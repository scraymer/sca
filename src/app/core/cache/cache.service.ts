...
import { CacheConstant } from './cache.constants';
...
export class CacheService implements OnDestroy {
  ...
  constructor(private updateService: SwUpdate, private snackBarService: MatSnackBar) {
    ...
    // subscribe to service work updates and if available or required, trigger event
    this.subscriptions.add(this.updateService.available.subscribe(() => this.onUpdateAvailable()));
    this.subscriptions.add(this.updateService.unrecoverable.subscribe(() => this.onUpdateAvailable()));
  }
  ...
  private onUpdateAvailable(): void {
    ...
    // display a messsage letting the user know a new application version is available
